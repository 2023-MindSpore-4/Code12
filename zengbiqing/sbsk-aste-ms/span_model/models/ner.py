...
class NERTagger(Model):
    ...
    def __init__(
        ...
        self._n_labels = {
            ns: pretrain_transformer_indexer.get_voc_size(ns)
        ...
        for ns in self._namespaces:
            null_label = pretrain_transformer_indexer("", ns)
            assert (
                null_label == 0
            )  , f"需要正确的空标签位置为 0 但是得到的是 {null_label}"
        ...
        self._ner_scorers = nn.ModuleDict()
        self._ner_metrics = {}
        ...
        for namespace in self._namespaces:
            ...
            self.i_opinion = pretrain_transformer_indexer("OPINION", namespace)
            self.i_target = pretrain_transformer_indexer("TARGET", namespace)
            if self.use_single_pool:
        ...
        if neg_class_weight != -1:
            ...
            self._loss = nn.CrossEntropyLoss(reduction="sum", weight=torch.tensor(weight))
        ...
        self._active_namespace = f"{data_set_name}__{self._name}"
    ...
    def forward(
        ...
        ner_scores[..., 0] = util.replace_masked_values(ner_scores[..., 0], span_mask.to(dtype=torch.bool), 1e20)
        ...
        output_dict.update(opinion_scores=torch.softmax(ner_scores, -1)[..., [self.i_opinion]])
        output_dict.update(target_scores=torch.softmax(ner_scores, -1)[..., [self.i_target]])

        if ner_labels is not None:
            if self.use_single_pool:
                ner_labels = torch.ne(ner_labels, 0.0).to(dtype=torch.int32)
            ...
            mask_flat = span_mask.view(-1).to(dtype=torch.bool)
    ...
    def predict(self, ner_scores, spans, span_mask):
        ...
        predictions = []
        zipped = zip(ner_scores, spans, span_mask)
        for ner_scores_sent, spans_sent, span_mask_sent in zipped:
            ...
            for label, label_score_raw, label_score_softmax, label_span in zip_pred:
                label_str = self.pretrain_transformer_indexer.get_str_of_idx(label.item(), self._active_namespace)
                ...
                prediction = ner        # without str input
