...
class AFLW2000(Dataset):
    ...
    def __getitem__(self, index):
        ...
        labels = np.array([yaw, pitch, roll], dtype=np.float32)
        ...
        return img, ms.Tensor(R, dtype=ms.float32), labels, self.X_train[index]
...
class AFW(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # Bin values
        bins = np.array(range(-99, 102, 3))
        labels_np = np.digitize([yaw, pitch, roll], bins) - 1
        labels = ms.Tensor(labels_np, dtype=ms.int32)
        cont_labels_np = np.array([yaw, pitch, roll], dtype=np.float32)
        cont_labels = ms.Tensor(cont_labels_np, dtype=ms.float32)
...
class BIWI(Dataset):
    ...
    def __getitem__(self, index):
        ...
        cont_labels = ms.Tensor(cont_labels, dtype=ms.float32)
        ...
        cont_labels = np.array([yaw, pitch, roll], dtype=np.float32)
        cont_labels = ms.Tensor(cont_labels, dtype=ms.float32)
        return img, ms.Tensor(R, dtype=ms.float32), cont_labels, self.X_train[index]
...
class Pose_300W_LP:
    ...
    def __init__(self, data_dir, filename_path, img_ext='.jpg', annot_ext='.mat', image_mode='RGB', shuffle=True, num_workers=4):
        self.data_dir = data_dir
        self.img_ext = img_ext
        self.annot_ext = annot_ext
        filename_list = get_list_from_filenames(filename_path)
    ...
    def generator(self):
        for index in range(len(self.X_train)):
            img = Image.open(os.path.join(
                self.data_dir, self.X_train[index][2:] + '.jpg'))
            img = img.convert(self.image_mode)

            mat_path = os.path.join(
                self.data_dir, self.y_train[index] + '.mat')
            ...
            yield img, R, [], self.X_train[index]

    def get_dataset(self, batch_size=32, num_parallel_workers=4, shuffle=True):
        # Create a GeneratorDataset object
        dataset = ds.GeneratorDataset(list(self.generator()), column_names=["image", "R", "empty", "filename"])

        # Define transformations
        image_trans = [
            vision.RandomResizedCrop(size=224, scale=(0.8, 1)),
            vision.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])
        ]
        # Apply transformations to images
        dataset = dataset.map(input_columns=["image"], operations=image_trans, num_parallel_workers=num_parallel_workers)

        # Set batch size and shuffle
        dataset = dataset.batch(batch_size, drop_remainder=True)
        if shuffle:
            dataset = dataset.shuffle(buffer_size=self.length)
