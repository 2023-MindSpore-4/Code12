    ...
    def __getitem__(self, index):
        while True:
            ...
            if cfg.TRAIN.MODEL_TYPE == 'SEP':
                ...
                template = template.swapaxes((2, 0, 1)).astype(np.float32)
                search = search.swapaxes((2, 0, 1)).astype(np.float32)
                return {
                    'template': template,
            ...
            #for e2e
                ...
                template = template.swapaxes((2, 0, 1)).astype(np.float32)
                search = search.swapaxes((2, 0, 1)).astype(np.float32)
                template_lp = template_lp.swapaxes((2, 0, 1)).astype(np.float32)
                template_hm = template_hm.swapaxes((2, 0, 1)).astype(np.float32)
                search_hm = search_hm.swapaxes((2, 0, 1)).astype(np.float32)
