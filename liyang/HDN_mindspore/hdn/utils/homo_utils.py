...
def transformer(U, theta, out_size, **kwargs):
    ...
    def _repeat(x, n_repeats):
        ...
        rep = ops.ones([n_repeats, ]).unsqueeze(0)
        rep = rep.int()
        x = x.int()
    ...
    def _meshgrid(height, width, scale_h):
        ...
        if scale_h:
            x_t = ops.matmul(ops.ones([height, 1]),
                               ops.transpose(ops.expand_dims(ops.linspace(-1.0, 1.0, width), 1), (1, 0)))
            y_t = ops.matmul(ops.expand_dims(ops.linspace(-1.0, 1.0, height), 1),
                               ops.ones([1, width]))
        else:
            x_t = ops.matmul(ops.ones([height, 1]),
                               ops.transpose(ops.expand_dims(ops.linspace(0.0, width.float(), width), 1), (1, 0)))
            y_t = ops.matmul(ops.expand_dims(ops.linspace(0.0, height.float(), height), 1),
                               ops.ones([1, width]))
    ...
    def _transform(theta, input_dim, out_size, scale_h):
        ...
        # smaller
        small = 1e-7
        smallers = 1e-6 * (1.0 - ops.ge(ops.abs(t_s_flat), small).float())

        t_s_flat = t_s_flat + smallers
        condition = ops.ReduceSum()(ops.gt(ops.abs(t_s_flat), small).float())
        # Ty changed
