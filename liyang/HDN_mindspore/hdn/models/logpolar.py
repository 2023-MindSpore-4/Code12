...
class STN_Polar(nn.Cell):
    ...
    def _prepare_grid(self, sz, delta):
        ...
        # rho = (ops.exp(mag * x_ls) - 1.0) + delta[0]
...
class STN_LinearPolar(nn.Cell):
    ...
    def forward(self, x, polar):
        ...
        # x = ops.grid_sample(x, grid, mode='bilinear', padding_mode='border', align_corners=False)
        x = ops.grid_sample(x, grid, mode='bilinear', padding_mode='border')
...
class Polar_Pick(nn.Cell):
    ...
    def _getSoftArgMax(self, r):
        ...
        x_ls = ops.linspace(0, sizes[1] - 1, sizes[1])
        y_ls = ops.linspace(0, sizes[2] - 1, sizes[2])
        x, y = ops.meshgrid([x_ls, y_ls])
