...
class LBHinge(nn.Cell):
    ...
    def forward(self, prediction, label, target_bb=None):
        ...
        if self.clip is not None:
            loss = ops.min(loss, ms.tensor([self.clip], device=loss.device))
        return loss


class LBHingev2(nn.Cell):
    """Loss that uses a 'hinge' on the lower bound.
    This means that for samples with a label value smaller than the threshold, the loss is zero if the prediction is
    also smaller than that threshold.
    args:
...
class IsTargetCellLoss(nn.Cell):
...
class TrackingClassificationAccuracy(nn.Cell):
    ...
    def forward(self, prediction, label, valid_samples=None):
        ...
        label_val_at_peak = label_reshaped[ops.arange(len(argmax_id)), argmax_id]
        label_val_at_peak = ops.max(label_val_at_peak, ops.zeros_like(label_val_at_peak))
