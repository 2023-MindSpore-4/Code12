...
def apply_filter(feat, filter, dilation_factors=None):
    ...
    if multiple_filters:
        if dilation_factors is None:
            scores = ops.conv2d(feat.reshape(num_images, -1, feat.shape[-2], feat.shape[-1]), filter.view(-1, *filter.shape[-3:]),
            ...
            for d_factor, num_filters_with_d in dilation_factors.items():
                ...
                scores_d = ops.conv2d(feat.reshape(num_images, -1, feat.shape[-2], feat.shape[-1]),
            ...
            scores = ops.cat(scores_all, dim=2)
            return scores

    scores = ops.conv2d(feat.reshape(num_images, -1, feat.shape[-2], feat.shape[-1]), filter,
...
def _apply_filter_ksz1(feat, filter):
    ...
    scores = ops.matmul(filter.reshape(num_sequences, 1, 1, num_channels),
...
def _apply_feat_transpose_v2(feat, input, filter_ksz):
    ...
    if multiple_filters:
        filter_grad = ops.conv2d(input.reshape(-1, num_filters, input.shape[-2], input.shape[-1]).permute(1,0,2,3),
    ...
    filter_grad = ops.conv2d(input.reshape(1, -1, input.shape[-2], input.shape[-1]),
                                     feat.reshape(-1, 1, feat.shape[-2], feat.shape[-1]),
                                     padding=trans_pad, groups=num_images * num_sequences)
...
def _apply_feat_transpose_v3(feat, input, filter_ksz):
    ...
    filter_grad = ops.conv2d(feat.reshape(-1, feat.shape[-3], feat.shape[-2], feat.shape[-1]).permute(1,0,2,3),
...
def _apply_feat_transpose_v4(feat, input, filter_ksz):
    ...
    filter_grad = ops.conv2d(feat.permute(2,1,0,3,4).reshape(feat.shape[-3], -1, feat.shape[-2], feat.shape[-1]),
