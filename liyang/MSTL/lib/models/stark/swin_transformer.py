...
import os
from .swin_config import get_config_new
...
class WindowAttention(nn.Cell):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        self.relative_position_bias_table = nn.Parameter(
            ops.zeros((2 * window_size[0] - 1) * (2 * window_size[1] - 1), num_heads))  # 2*Wh-1 * 2*Ww-1, nH
...
class BasicLayer(nn.Cell):
    ...
    def forward(self, x, H, W):
        ...
        img_mask = ops.zeros((1, Hp, Wp, 1), device=x.device)  # 1 Hp Wp 1
...
class SwinTransformer(nn.Cell):
    ...
    def __init__(self,
        ...
        # stochastic depth
        dpr = [x.item() for x in ops.linspace(0, drop_path_rate, sum(depths))]  # stochastic depth decay rule
