...
from lib.models.mstl.backbone import FrozenBatchNorm2d
from lib.models.mstl.repvgg import RepVGGBlock
# import time
...
class Corner_Predictor(nn.Cell):
    ...
    def __init__(self, inplanes=64, channel=256, feat_sz=20, stride=16, freeze_bn=False):
        ...
        # generate mesh-grid
        self.coord_x = self.indice.tile((self.feat_sz, 1)) \
            .view((self.feat_sz * self.feat_sz,)).float()
        self.coord_y = self.indice.tile((1, self.feat_sz)) \
            .view((self.feat_sz * self.feat_sz,)).float()
    ...
    def forward(self, x, return_dist=False, softmax=True):
        ...
        if return_dist:
            ...
            return ops.stack((coorx_tl, coory_tl, coorx_br, coory_br), dim=1) / self.img_sz, prob_vec_tl, prob_vec_br
        else:
            coorx_tl, coory_tl = self.soft_argmax(score_map_tl)
            coorx_br, coory_br = self.soft_argmax(score_map_br)
            return ops.stack((coorx_tl, coory_tl, coorx_br, coory_br), dim=1) / self.img_sz
...
class Corner_Predictor_Lite(nn.Cell):
    ...
    def __init__(self, inplanes=64, channel=256, feat_sz=20, stride=16):
        ...
        self.conv_tower = nn.SequentialCell(conv(inplanes, channel),
        ...
        self.indice = (ops.arange(0, self.feat_sz).view(-1, 1) + 0.5) * self.stride  # here we can add a 0.5
        # generate mesh-grid
        self.coord_x = self.indice.tile((self.feat_sz, 1)) \
            .view((self.feat_sz * self.feat_sz,)).float()
        self.coord_y = self.indice.tile((1, self.feat_sz)) \
...
class Corner_Predictor_Lite_Rep(nn.Cell):
    ...
    def __init__(self, inplanes=64, channel=256, feat_sz=20, stride=16):
        ...
        self.img_sz = self.feat_sz * self.stride
        '''convolution tower for two corners'''
        self.conv_tower = nn.SequentialCell(RepVGGBlock(inplanes, channel, kernel_size=3, padding=1),
...
class Corner_Predictor_Lite_Rep_v2(nn.Cell):
    """ Corner Predictor module (Lite version with repvgg style)"""
