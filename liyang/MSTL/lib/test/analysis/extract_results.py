...
def calc_iou_overlap(pred_bb, anno_bb):
    tl = ops.max(pred_bb[:, :2], anno_bb[:, :2])
    br = ops.min(pred_bb[:, :2] + pred_bb[:, 2:] - 1.0, anno_bb[:, :2] + anno_bb[:, 2:] - 1.0)
    ...
    if pred_bb.shape[0] != anno_bb.shape[0]:
        if dataset == 'lasot':
            ...
            if pred_bb.shape[0] > anno_bb.shape[0]:
                ...
                pad = ops.zeros((anno_bb.shape[0] - pred_bb.shape[0], 4)).type_as(pred_bb)
                pred_bb = ops.cat((pred_bb, pad), dim=0)
...
def extract_results(trackers, dataset, report_name, skip_missing_seq =False, plot_bin_gap=0.05,
    ...
    threshold_set_overlap = ops.arange(0.0, 1.0 + plot_bin_gap, plot_bin_gap, dtype=torch.float64)
    threshold_set_center = ops.arange(0, 51, dtype=torch.float64)
    threshold_set_center_norm = ops.arange(0, 51, dtype=torch.float64) / 100.0

    avg_overlap_all = ops.zeros((len(dataset), len(trackers)), dtype=torch.float64)
    ave_success_rate_plot_overlap = ops.zeros((len(dataset), len(trackers), threshold_set_overlap.numel()),
                                                dtype=ms.float32)
    ave_success_rate_plot_center = ops.zeros((len(dataset), len(trackers), threshold_set_center.numel()),
                                               dtype=ms.float32)
    ave_success_rate_plot_center_norm = ops.zeros((len(dataset), len(trackers), threshold_set_center.numel()),
                                                    dtype=ms.float32)

    valid_sequence = ops.ones(len(dataset), dtype=torch.uint8)

    for seq_id, seq in enumerate(tqdm(dataset)):
        # Load anno
        anno_bb = ms.tensor(seq.ground_truth_rect)
        target_visible = ms.tensor(seq.target_visible, dtype=torch.uint8) if seq.target_visible is not None else None
        for trk_id, trk in enumerate(trackers):
            ...
            if os.path.isfile(results_path):
                pred_bb = ms.tensor(load_text(str(results_path), delimiter=('\t', ','), dtype=np.float64))
            else:
                if skip_missing_seq:
