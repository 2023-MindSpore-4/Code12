...
def merge_multiple_runs(eval_data):
    ...
    ave_success_rate_plot_overlap = ms.tensor(eval_data['ave_success_rate_plot_overlap'])
    ave_success_rate_plot_center = ms.tensor(eval_data['ave_success_rate_plot_center'])
    ave_success_rate_plot_center_norm = ms.tensor(eval_data['ave_success_rate_plot_center_norm'])
    avg_overlap_all = ms.tensor(eval_data['avg_overlap_all'])

    trackers = eval_data['trackers']
    merged = ops.zeros(len(trackers), dtype=torch.uint8)
    ...
    ave_success_rate_plot_overlap_merged = ops.stack(ave_success_rate_plot_overlap_merged, dim=1)
    ave_success_rate_plot_center_merged = ops.stack(ave_success_rate_plot_center_merged, dim=1)
    ave_success_rate_plot_center_norm_merged = ops.stack(ave_success_rate_plot_center_norm_merged, dim=1)
    avg_overlap_all_merged = ops.stack(avg_overlap_all_merged, dim=1)
...
def plot_results(trackers, dataset, report_name, merge_results=False,
    ...
    valid_sequence = ms.tensor(eval_data['valid_sequence'], dtype=torch.bool)
    ...
    if 'success' in plot_types:
        ...
        threshold_set_overlap = ms.tensor(eval_data['threshold_set_overlap'])
    ...
    if 'prec' in plot_types:
        ...
        threshold_set_center = ms.tensor(eval_data['threshold_set_center'])
    ...
    if 'norm_prec' in plot_types:
        ...
        threshold_set_center_norm = ms.tensor(eval_data['threshold_set_center_norm'])
...
def plot_got_success(trackers, report_name):
    ...
    auc_curve = ops.zeros((len(trackers), 101))
    scores = ops.zeros(len(trackers))
    ...
    for trk_id, trk in enumerate(trackers):
        ...
        auc_curve[trk_id, :] = ms.tensor(curve) * 100.0
    ...
    threshold_set_overlap = ops.arange(0.0, 1.01, 0.01, dtype=torch.float64)
...
def print_per_sequence_results(trackers, dataset, report_name, merge_results=False,
    ...
    avg_overlap_all = ms.tensor(eval_data['avg_overlap_all']) * 100.0
