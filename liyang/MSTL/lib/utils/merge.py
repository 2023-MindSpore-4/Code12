...
def merge_template_search(inp_list, return_search=False, return_template=False):
    ...
    seq_dict = {"feat": ops.cat([x["feat"] for x in inp_list], dim=0),
                "mask": ops.cat([x["mask"] for x in inp_list], dim=1),
                "pos": ops.cat([x["pos"] for x in inp_list], dim=0)}
    if return_search:
        x = inp_list[-1]
...
def get_qkv(inp_list):
    ...
    dict_x = inp_list[-1]
    dict_c = {"feat": ops.cat([x["feat"] for x in inp_list], dim=0),
              "mask": ops.cat([x["mask"] for x in inp_list], dim=1),
              "pos": ops.cat([x["pos"] for x in inp_list], dim=0)}  # concatenated dict
