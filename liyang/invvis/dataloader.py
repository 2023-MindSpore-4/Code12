...
class DHNDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.isTrain:
            ...
            # randData = randData + ops.randn_like(randData) * 0.05

            randDataMin = ops.min(randData)
            randDataMax = ops.max(randData)
            ...
            for i in range(GetOption("embedChannelNum") - 2):
                ...
                # now = now + ops.randn_like(now) * 0.05

                nowMin = ops.min(now)
                nowMax = ops.max(now)
                now = (now - nowMin) / (nowMax - nowMin + 0.00001)

                randData = ops.cat([randData, now], dim=0)
            ...
            randData = ops.cat([randData, qr * GetOption("qrMul")], dim=0)
