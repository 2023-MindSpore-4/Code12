...
trainDataLoader = GeneratorDataset(trainDataSet, batch_size=GetOption("batchSize"), shuffle=True, num_workers=16,
                             drop_last=True)
...
for ec in range(GetOption("epochNum")):
    ...
    for i, data in enumerate(trainDataLoader):
        ...
        netInput = ops.cat((img, randData), axis=1)
    ...
    if epochCount > 0 and (epochCount + 1) % 10 == 0:
        ms.save({'opt': optimizer.state_dict(),
    ...
    lrScheduler.step()
        ...
        randData = ops.cat([dataX, dataY, dataW * GetOption("qrMul")], axis=0).cuda()
