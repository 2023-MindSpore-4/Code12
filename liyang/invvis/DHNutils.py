...
def Differentiable_Round(x, alpha):
    m = ops.floor(x) + 0.5
    r = x - m
    z = ops.tanh(ms.Tensor([alpha / 2.0])) * 2.0
    y = m.cuda() + (ops.tanh(alpha * r.cuda()) / z.cuda()).cuda()
...
def InitializeModel(mod):
    for key, param in mod.named_parameters():
        ...
        if param.requires_grad:
            param.data = 0.01 * ops.randn(param.data.shape).cuda()
            if split[-2] == 'conv5':
                param.data.fill_(0.)
...
def Unnormalize(tensor: ms.Tensor, mean, std, inplace: bool = False) -> ms.Tensor:
    ...
    dtype = tensor.dtype
    mean = ops.as_tensor(mean, dtype=dtype, device=tensor.device)
    std = ops.as_tensor(std, dtype=dtype, device=tensor.device)
    if (std == 0).any():
...
def SaveScatterImg(name):
    ...
    img = ops.randn([size * size, 2]).numpy()
    ...
    # print(img.shape)
    imgX = ms.tensor(img).reshape(512, 512, 2).permute(2, 0, 1)[0:1, ...].unsqueeze(0)
    imgY = ms.tensor(img).reshape(512, 512, 2).permute(2, 0, 1)[1:2, ...].unsqueeze(0)
...
def SaveScatterImgV2(name, m):
    img = ops.randn([512 // m * 512 // m, 2]).numpy()
    img = (img - np.min(img)) / (np.max(img) - np.min(img))
    img = img.tolist()
...
def SaveScatterImgV3(name, m):
    img = ops.randn([512 // m + 1,  512 // m + 1]).numpy()
    img = (img - np.min(img)) / (np.max(img) - np.min(img))
    img = img.tolist()
    ...
    # print(img.shape)
    img = ms.tensor(img).reshape(512, 512, 1).permute(2, 0, 1).unsqueeze(0)
...
def SaveQRCode(name):
    ...
    qrImg = ms.tensor(np.array(qr)).unsqueeze(0).unsqueeze(0).float()
...
def GetImgFromPatch(imgSize, patchSize, patchList):
    ...
    for i in range(hc):
        row = patchList[i * wc]
        for j in range(1, wc):
            row = ops.cat([row, patchList[i * wc + j]], dim=3)
        if i == 0:
            ret = row
        else:
            ret = ops.cat([ret, row], dim=2)
