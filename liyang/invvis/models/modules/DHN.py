...
def gauss_noise(shape):
    noise = ops.zeros(shape).cuda()
    for i in range(noise.shape[0]):
        noise[i] = ops.randn(noise[i].shape).cuda()
...
class EnhanceBlock(nn.Cell):
    def __init__(self, inChannelNum, middleChannelNum1, middleChannelNum2):
        ...
        self.stage1 = nn.SequentialCell(
            ...
            nn.Conv2d(middleChannelNum1, middleChannelNum1, kernel_size=1, stride=1, padding=0, has_bias=True),
            nn.Conv2d(middleChannelNum1, middleChannelNum1, kernel_size=3, stride=1, padding=1, has_bias=True),
            nn.Conv2d(middleChannelNum1, middleChannelNum1, kernel_size=1, stride=1, padding=0, has_bias=True),
        )
        self.stage2 = nn.SequentialCell(
            DenseBlock(middleChannelNum1, middleChannelNum2),
            nn.Conv2d(middleChannelNum2, middleChannelNum2, kernel_size=1, stride=1, padding=0, has_bias=True),
            nn.Conv2d(middleChannelNum2, middleChannelNum2, kernel_size=3, stride=1, padding=1, has_bias=True),
            nn.Conv2d(middleChannelNum2, middleChannelNum2, kernel_size=1, stride=1, padding=0, has_bias=True),
        )
...
class DHN(nn.Cell):
    ...
    def decode(self, embed):
        noise = ops.zeros(embed.shape[0], self.embedChannelNum * 4, embed.shape[2] // 2, embed.shape[3] // 2).cuda()
        restoreInputImg = self.haar(embed, False)

        restoreInputData = noise
        restoreInput = ops.cat([restoreInputImg, restoreInputData], dim=1)
        ...
        restore = ops.cat([restoreT, restoreB], dim=1)
    ...
    def decodeNoHaar(self, embed):
        noise = ops.zeros(embed.shape[0], self.embedChannelNum, embed.shape[2], embed.shape[3]).cuda()
        restoreInputImg = embed.clone()

        restoreInputData = noise
        restoreInput = ops.cat([restoreInputImg, restoreInputData], axis=1)
