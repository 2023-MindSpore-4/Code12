...
class DeformModel(nn.Cell):
    ...
    def construct(self, x, zapp, zgeo):
        x_hat, _, geo = self.decode(zapp, zgeo)
        loss = 1.0 / (2.0 * self.sigma * self.sigma) * self.mse(x_hat, x)
        return loss


def gradient_loss(gen_frames, alpha=1):
    ...
    def gradient(x):
        ...
        # gradient step=1
        left = x
        right = ops.pad(x, [0, 1, 0, 0])[:, :, :, 1:]
        top = x
        bottom = ops.pad(x, [0, 0, 0, 1])[:, :, 1:, :]
    ...
    return ops.mean(gen_dx ** alpha + gen_dy ** alpha)
