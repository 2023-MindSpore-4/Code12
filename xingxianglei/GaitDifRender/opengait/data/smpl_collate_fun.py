...
class SMPLCollateFn(object):
    def __init__(self, label_set, sample_config):
        ...
        self.feature_num_label = ['aligned-sils',
                                  'ratios',
                                  'rgbs',
                                  'sils',
                                  'pred_xyz_17',
                                  ...
                                  'height',
                                  'width', ]
        self.feature_num = len(self.feature_num_label)
    ...
    def __call__(self, batch):
        ...
        def sample_frames(seqs):
            global count
            sampled_fras = {k: [] for k in self.feature_num_label}
            seq_len = seqs[self.feature_num_label[0]].shape[0]
        ...
        if self.sampler == "fixed":
            fras_batch_set = {}
            for feature_name in self.feature_num_label:
                fras_batch_set[feature_name] = [np.asarray(fras_batch[i][feature_name]) for i in range(batch_size)]
        else:
