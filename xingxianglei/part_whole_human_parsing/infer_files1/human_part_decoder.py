...
from infer_files1.stylegan_networks import ResUpBlock, GeneratorBlock
...
class HumanPartDecoder(nn.Cell):
    def __init__(self, relation_list=None, num_parts=14, app_dim=32, deform_dim=16, pose_dim=16, img_size=(128, 64)):
        ...
        app_fc_list = []
        app_fc_cell_list = nn.CellList()
        app_conv_list = []
        app_conv_cell_list = nn.CellList()
        for i in range(num_parts):
            ...
            app_fc_cell_list.append(fc)
            app_conv_list.append(conv)
            app_conv_cell_list.append(conv)
        self.insert_child_to_cell(child_name='part_app_fc_', child_cell=app_fc_cell_list)
        app_fc_cell_list.update_parameters_name('part_app_fc_')
        self.insert_child_to_cell(child_name='part_app_conv_', child_cell=app_conv_cell_list)
        app_conv_cell_list.update_parameters_name('part_app_conv_')
        self.app_fc_list = app_fc_list
        self.app_conv_list = app_conv_list
        deform_fc_list = []
        deform_fc_cell_list = nn.CellList()
        for i in range(num_parts):
            fc = nn.SequentialCell(
            ...
            # fc = PartDeformFC(deform_dim)
            # self.insert_child_to_cell(child_name=str(i), child_cell=fc)
            # fc.update_parameters_name('part_deform_fc_{}.'.format(i))
            # deform_fc.update_parameters_name('part_app_fc_{}.'.format(i))
            # deform_fc.update_cell_prefix()
            deform_fc_list.append(fc)
            deform_fc_cell_list.append(fc)
        self.insert_child_to_cell(child_name='part_deform_fc_', child_cell=deform_fc_cell_list)
        deform_fc_cell_list.update_parameters_name('part_deform_fc_')
        self.deform_fc_list = deform_fc_list
        pose_fc_list = []
        pose_fc_cell_list = nn.CellList()
        for i in range(num_parts):
            fc = nn.SequentialCell(
            ...
            # fc = PartPoseFC(pose_dim)
            # self.insert_child_to_cell(child_name=str(i), child_cell=pose_fc)
            # pose_fc.update_parameters_name('part_pose_fc_{}.'.format(i))
            # pose_fc.update_parameters_name('part_app_fc_{}.'.format(i))
            # pose_fc.update_cell_prefix()
            pose_fc_list.append(fc)
            pose_fc_cell_list.append(fc)
        self.insert_child_to_cell(child_name='part_pose_fc_', child_cell=pose_fc_cell_list)
        pose_fc_cell_list.update_parameters_name('part_pose_fc_')
        ...
        self.generator_block3 = GeneratorBlock(128, 64, upsample=False, upsample_rgb=False)
    ...
    def process_deform_field(self, z):
        ...
        for i, fc in enumerate(self.deform_fc_list):
            ...
            deform_field_i = deform_field_i.reshape(-1, 16, 8, 2)
